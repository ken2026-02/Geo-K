...
export class SqliteImportBatchRepository implements ImportBatchRepository {
  ...
  async findDuplicate(sourceReportId: string): Promise<ImportBatchRecord | undefined> {
    const row = readFirstRow(this.db, `SELECT * FROM import_batches WHERE source_report_id = '${escapeSqlString(sourceReportId)}' AND import_status = 'review_in_progress'`);
    return row ? this.map(row) : undefined;
  }
  ...
  async delete(id: string): Promise<void> {
    ...
    try {
      this.db.run(`DELETE FROM pending_item_image_links WHERE pending_image_id IN (SELECT id FROM pending_images WHERE batch_id = '${escapedId}')`);
      this.db.run(`DELETE FROM pending_images WHERE batch_id = '${escapedId}'`);
      this.db.run(`DELETE FROM pending_words WHERE batch_id = '${escapedId}'`);
      this.db.run(`DELETE FROM pending_phrases WHERE batch_id = '${escapedId}'`);
      this.db.run(`DELETE FROM pending_sentences WHERE batch_id = '${escapedId}'`);
      this.db.run(`DELETE FROM pending_geo_materials WHERE batch_id = '${escapedId}'`);
      this.db.run(`DELETE FROM pending_geo_features WHERE batch_id = '${escapedId}'`);
      this.db.run(`DELETE FROM pending_strategies WHERE batch_id = '${escapedId}'`);
      this.db.run(`DELETE FROM import_batches WHERE id = '${escapedId}'`);
      this.db.run("COMMIT;");
    } catch (error) {
  ...
  async deleteAll(): Promise<void> {
    this.db.run("BEGIN TRANSACTION;");
    try {
      this.db.run("DELETE FROM pending_item_image_links");
      this.db.run("DELETE FROM pending_images");
      this.db.run("DELETE FROM pending_words");
      this.db.run("DELETE FROM pending_phrases");
      this.db.run("DELETE FROM pending_sentences");
      this.db.run("DELETE FROM pending_geo_materials");
      this.db.run("DELETE FROM pending_geo_features");
      this.db.run("DELETE FROM pending_strategies");
      this.db.run("DELETE FROM import_batches");
      this.db.run("COMMIT;");
    } catch (error) {
      this.db.run("ROLLBACK;");
      throw error;
