...
export interface ImportBatchRepository extends BaseRepository<ImportBatchRecord> {
  ...
  findDuplicate(sourceReportId: string): Promise<ImportBatchRecord | undefined>;
}
...
export type LearningCollectionSourceType = "standard" | "manual" | "guide" | "report" | "code" | "personal";
export interface LearningCollectionRecord {
  id: string;
  name: string;
...
export interface LearningCollectionRepository extends BaseRepository<LearningCollectionRecord> {
  listAll(): Promise<LearningCollectionRecord[]>;
}
