    ...
    return this.performanceService.measure("commit", { batchId }, async () => {
      ...
      try {
        ...
        const committedStrategyIds = approvedStrategies.map((item) => {
          strategyRepository.upsertStrategy({
            id: item.id,
            canonicalName: item.rawName,
            normalizedName: item.normalizedName,
            chineseName: item.chineseName,
          ...
          pendingToApproved.set(`strategy:${item.id}`, item.id);

          if (reportId) {
            strategyRepository.upsertItemSource({
              id: `item-source:strategy:${item.id}:${batchId}`,
              itemType: "strategy",
              itemId: item.id,
              reportId,
              createdAt: timestamp
          ...
          return { id: item.id, normalizedName: item.normalizedName };
        });
        ...
        for (const feature of committedFeatureIds) {
          ...
          for (const strategy of committedStrategyIds) {
            if (feature.context.includes(strategy.normalizedName)) {
              strategyRepository.upsertRelation({
                id: `item-relation:feature-strategy:${feature.id}:${strategy.id}`,
                fromItemType: "geo_feature",
                fromItemId: feature.id,
                relationType: "recommended_strategy",
                toItemType: "strategy",
                toItemId: strategy.id,
