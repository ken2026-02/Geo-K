...
export class JsonKnowledgePackImportService {
  ...
  async importIntoPending(db: Database, sourceFileName: string, rawJson: string, imageFiles: Map<string, Blob> = new Map<string, Blob>()): Promise<JsonImportResult> {
    return this.performanceService.measure("import", { sourceFileName }, async () => {
      ...
      try {
        ...
        pendingStagingRepository.stageWords(this.mapWords(batch.id, prepared.pack));
        pendingStagingRepository.stagePhrases(this.mapPhrases(batch.id, prepared.pack));
        pendingStagingRepository.stageSentences(this.mapSentences(batch.id, prepared.pack));
        pendingStagingRepository.stageGeoMaterials(this.mapGeoMaterials(batch.id, prepared.pack));
        pendingStagingRepository.stageGeoFeatures(this.mapGeoFeatures(batch.id, prepared.pack));
        pendingStagingRepository.stageStrategies(this.mapStrategies(batch.id, prepared.pack));
        db.exec("COMMIT;");
      } catch (error) {
        db.exec("ROLLBACK;");
        throw new ImportExecutionError({
      ...
      const imageSummary = await this.processPendingImages(db, batch.id, prepared.pack, imageFiles, validationReport);
    ...
    for (const image of pack.images) {
      ...
      try {
        ...
        const processedAsset = await this.imagePipeline.process(
          { id: image.id, fileName: image.file_name, mimeType: preferredMime, blob: sourceBlob },
          false
        );
        ...
        for (const variant of processedAsset.variants) {
          const saved = await this.imageStorageService.saveVariant(batchId, image.id, variant.variant, variant.blob, variant.mimeType, variant.width, variant.height);
        ...
        image.linked_items.forEach((linkedItem, index) => {
          ...
          pendingLinks.push({
            ...
            pendingItemType,
            pendingItemId: linkedItem.item_id,
  ...
  private mapWords(batchId: string, pack: KnowledgePack): PendingWordStageRecord[] {
  ...
  private mapPhrases(batchId: string, pack: KnowledgePack): PendingPhraseStageRecord[] {
  ...
  private mapSentences(batchId: string, pack: KnowledgePack): PendingSentenceStageRecord[] {
  ...
  private mapGeoMaterials(batchId: string, pack: KnowledgePack): PendingGeoMaterialStageRecord[] {
  ...
  private mapGeoFeatures(batchId: string, pack: KnowledgePack): PendingGeoFeatureStageRecord[] {
  ...
  private mapStrategies(batchId: string, pack: KnowledgePack): PendingStrategyStageRecord[] {
