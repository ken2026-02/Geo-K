...
export type CommitItemType = "word" | "phrase" | "sentence" | "geo_material" | "geo_feature";
export type RelatedItemType = CommitItemType | "strategy";
...
export class SqliteApprovedLibraryRepository {
  ...
  listWords(): LibraryListItem[] {
    return readRows(this.db, "SELECT id, canonical_word, language_category, provenance_type, updated_at FROM words ORDER BY updated_at DESC").map((row) => ({
      id: String(row.id),
  ...
  listPhrases(): LibraryListItem[] {
    return readRows(this.db, "SELECT id, canonical_phrase, function_type, provenance_type, updated_at FROM phrases ORDER BY updated_at DESC").map((row) => ({
      id: String(row.id),
  ...
  listSentences(): LibraryListItem[] {
    return readRows(this.db, "SELECT id, canonical_sentence, sentence_type, provenance_type, updated_at FROM sentences ORDER BY updated_at DESC").map((row) => ({
      id: String(row.id),
  ...
  listRelations(itemType: CommitItemType, id: string): RelatedItemRecord[] {
    const outgoing = readRows(
      ...
      SELECT r.relation_type, r.confidence_score,
             ...
             COALESCE(w.canonical_word, p.canonical_phrase, s.canonical_sentence, gm.canonical_name, gf.canonical_name, st.canonical_name) AS related_title,
             COALESCE(w.language_category, p.function_type, s.sentence_type, gm.geo_material_category, gf.geo_feature_category, st.strategy_category) AS related_subtitle
      ...
      WHERE r.from_item_type = '${escapeSqlString(itemType)}' AND r.from_item_id = '${escapeSqlString(id)}'
        AND COALESCE(w.id, p.id, s.id, gm.id, gf.id, st.id) IS NOT NULL
