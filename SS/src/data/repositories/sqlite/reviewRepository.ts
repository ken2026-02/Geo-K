...
import type { CommitItemType } from "./approvedLibraryRepository";

export type ReviewableItemType = CommitItemType;
...
export class SqliteReviewRepository {
  ...
  getStats(reviewMode?: string): ReviewStatsRecord {
    const reviewModeFilter = reviewMode ? `WHERE review_mode = '${escapeSqlString(reviewMode)}'` : "";
    const rows = readRows(this.db, `SELECT review_result, COUNT(*) AS total_count FROM review_logs ${reviewModeFilter} GROUP BY review_result`);

    const stats: ReviewStatsRecord = {
      total: 0,
      correct: 0,
      incorrect: 0,
      skipped: 0,
