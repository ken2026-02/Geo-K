...
async function main(): Promise<void> {
  ...
  assert.equal(importResult.batch.id, "pack-test-001");
  ...
  const systemInvalidReport = systemValidator.validate('{"schema_version":"1.0","pack_id":"bad","ownership":"system"}');
  ...
  assert.equal(pwaConfigRaw.includes('injectRegister: false'), true, "vite PWA config should disable delayed auto-injected registration");
