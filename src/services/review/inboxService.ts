...
export class InboxService {
  ...
  async deleteBatch(db: Database, id: string): Promise<void> {
    await new SqliteImportBatchRepository(db).delete(id);
    await persistDatabase(db);
  }

  async clearAllBatches(db: Database): Promise<void> {
    await new SqliteImportBatchRepository(db).deleteAll();
    await persistDatabase(db);
  }
  ...
  async approveAll(db: Database, batchId: string, itemTypeFilter?: string): Promise<void> {
    const repository = new SqlitePendingInboxRepository(db);
    const imageRepository = new SqlitePendingImageRepository(db);
    const items = await repository.listByBatch(batchId);
    const images = imageRepository.listByBatch(batchId);
    
    db.exec("BEGIN TRANSACTION;");
    try {
      for (const item of items) {
        if (item.reviewStatus === "unreviewed" || item.reviewStatus === "deferred") {
          if (!itemTypeFilter || itemTypeFilter === "all" || item.itemType === itemTypeFilter) {
            await repository.updateReviewStatus(item.itemType, item.id, "approved");
          }
        }
      }

      // Also approve images if no specific item type filter is applied or if "all" is selected
      if (!itemTypeFilter || itemTypeFilter === "all") {
        for (const image of images) {
          if (image.reviewStatus === "unreviewed" || image.reviewStatus === "deferred") {
            imageRepository.updateReviewStatus(image.id, "approved");
          }
        }
      }
